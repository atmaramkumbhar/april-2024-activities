...
const Registration = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
     axios.post('http://localhost:9091/loan/register', formData)
     .then(response => {alert(`Registered successfully with emailId:: + ${formData.emailId}`);navigate(`/login`)})
     .catch(error => alert("Record Already Exist"));
